-- Equipment schema validation helpers. Mirrors martial/inventory style.
...
local function invalid(schema_name, field, reason, details)
    ...
    return result_err(
        ...
        'error.equipment.schema_validation_failed',
        false,
        details
    )
end
...
local function non_empty_string(schema_name, field, value)
    if type_value(value) ~= 'string' or value == '' or #value > 128 then
...
local function sorted_unique_content_ids(schema_name, field, value, prefix, optional)
...
local function error_summary(err)
    ...
    local summary = {
        ...
        message_key = err_is_table and raw_get(err, 'message_key') or 'error.unknown',
        retryable = err_is_table and raw_get(err, 'retryable') == true,
    }
    local source_details = err_is_table and raw_get(err, 'details') or nil
    if type_value(source_details) ~= 'table' or get_metatable(source_details) ~= nil then
        return summary
    end
    local details = {}
    local key
