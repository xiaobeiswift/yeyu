-- Traversal schema validation helpers (system 26). Mirrors world/dialogue style.
...
local function invalid(schema_name, field, reason, details)
    ...
    return result_err(
        ...
        'error.traversal.schema_validation_failed',
        false,
        details
    )
end
...
local function dense_array(schema_name, field, value, minimum, maximum)
    ...
    if minimum ~= nil and length < minimum then
        return invalid(schema_name, field, 'ARRAY_TOO_SHORT', {
            minimum = minimum,
            actual = length,
        })
    end
    if maximum ~= nil and length > maximum then
        return invalid(schema_name, field, 'ARRAY_TOO_LONG', {
            maximum = maximum,
            actual = length,
        })
    end
