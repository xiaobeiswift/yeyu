...
function RuntimeId.validate_component(value, field_name)
    if not is_component(value) then
        return Result.err('ID_INVALID', 'error.foundation.runtime_id_invalid', false, {
            field = field_name,
...
function RuntimeId.validate_derived(value, field_name)
    if type(value) ~= 'string'
        ...
        or value:sub(-1) == ':'
    then
        return Result.err('ID_INVALID', 'error.foundation.derived_id_invalid', false, {
            field = field_name,
            max_bytes = DERIVED_MAX_BYTES,
        })
    end

    local receipt_namespace, receipt_digest = value:match(
    ...
    for component in value:gmatch('[^:]+') do
        if not is_component(component) then
            return Result.err('ID_INVALID', 'error.foundation.derived_id_component_invalid', false, {
                field = field_name,
                component = component,
...
function RuntimeId.validate_content(value, prefix, field_name)
    ...
    then
        return Result.err('ID_INVALID', 'error.foundation.content_id_invalid', false, {
            field = field_name,
            prefix = prefix,
...
local function validate_reference(value, maximum_bytes, field_name)
    if type(value) ~= 'string'
        ...
        or value:sub(-1) == ':'
    then
        return Result.err('ID_INVALID', 'error.foundation.reference_key_invalid', false, {
            field = field_name,
    ...
    for component in value:gmatch('[^:]+') do
        if #component > CONTENT_MAX_BYTES
            or component:match('^[A-Za-z0-9][A-Za-z0-9_.%-]*$') == nil
        then
            return Result.err(
                'ID_INVALID',
...
function RuntimeId.compose(parts)
    if type(parts) ~= 'table' or #parts < 2 then
        return Result.err('ID_INVALID', 'error.foundation.derived_id_parts_invalid', false)
    end
    ...
    for i = 1, #parts do
        ...
        if type(part) == 'number' then
            local encoded = DecimalInteger.encode(part)
            if part < 1 or encoded == nil then
                return Result.err('ID_INVALID', 'error.foundation.derived_id_number_invalid', false, {
                    part_index = i,
                })
            end
            part = encoded
        ...
        local checked = RuntimeId.validate_component(part, 'part_' .. tostring(i))
    ...
    if #value > DERIVED_MAX_BYTES then
        return Result.err('ID_TOO_LONG', 'error.foundation.derived_id_too_long', false, {
            max_bytes = DERIVED_MAX_BYTES,
        })
    end
    return Result.ok(value)
