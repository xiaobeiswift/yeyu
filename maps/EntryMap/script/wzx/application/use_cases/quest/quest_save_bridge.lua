-- System 14 quest slot-2 save bridge.
-- Persists quest_* sections through SaveCoordinator checkpoints.
-- Systems never call SaveStore; section merge never whole-slot overwrites foreign keys.
...
Bridge.__newindex = function()
    error_value('quest save bridge is read-only', 2)
end
...
local function fail(code, reason, details, retryable)
    ...
    return result_err(
        code,
        'error.quest.save_bridge_' .. string.lower(code),
        retryable == true,
        details
    )
end
...
local function ensure_slot1_context(state, player_ref, player_save_scope, request_id, input)
    ...
    if state.auto_create_save ~= true then
        return fail(
            'SAVE_NOT_READY',
            'SLOT1_REQUIRED_BEFORE_QUEST_PERSIST',
...
function Bridge:persist_player_quest(input)
    ...
    local request_id = raw_get(input, 'request_id') or 'request_quest_save'
    ...
    if type_value(bundle) ~= 'table' or get_metatable(bundle) ~= nil then
        return invalid_argument('QUEST_BUNDLE_INVALID', { field = 'bundle' })
    end
    if type_value(bundle.quest_metadata) ~= 'table' then
        return invalid_argument('QUEST_METADATA_REQUIRED', {
            field = 'bundle.quest_metadata',
        })
    end
    ...
    -- Slot 2 is shared; merge only quest-owned sections.
    local merged_slot2 = SlotPayloadUtil.merge_sections(slot2.value.payload, {
        quest_metadata = bundle.quest_metadata,
        quest_runs = bundle.quest_runs,
        quest_objectives = bundle.quest_objectives,
        quest_event_receipts = bundle.quest_event_receipts,
        revealed_hidden_quests = bundle.revealed_hidden_quests,
        tracked_quest_runs = bundle.tracked_quest_runs,
    })
...
function QuestSaveBridge.bind(options)
...
function QuestSaveBridge.is_authority(value)
    return STATES[value] ~= nil
end

return QuestSaveBridge
